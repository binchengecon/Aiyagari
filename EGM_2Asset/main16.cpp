// Add utility: utility of wealth u(a) instead of u(a')
...
void POLICY(double *VF_final, double *dVF_final, double *save_final, double *VF, double *dVF, double *save, double *Portfolio, double K[size_asset], double Omega[size_portfoliochoice + 1], double wagerate)
{
    ...
    VFendo = (double *)calloc((ARRLLP_dim), sizeof(double)); // Value function on the next time grid, next iteration
    VFnew = (double *)calloc((ARRLLP_dim), sizeof(double));  // Value function on the next time grid, next iteration

    VF_final_old = (double *)calloc((ARRLL_dim), sizeof(double)); // Value function on the next time grid, next iteration
    // Kendo = (double *)calloc((ARRLLP_dim), sizeof(double));  // endogenous grid values
    // Kendo_min = (double *)calloc((maxygrid), sizeof(double)); // endogenous grid values
    // eVF = (double *)calloc((ARRLLP_dim), sizeof(double));       // expected value function
    // deVF = (double *)calloc((ARRLLP_dim), sizeof(double));      // derivative of the expected value function
    cohendo = (double *)calloc((ARRLLP_dim), sizeof(double));  // Value function on the next time grid, next iteration
    consendo = (double *)calloc((ARRLLP_dim), sizeof(double)); // Value function on the next time grid, next iteration
    ...
    while (critV > epsV && iter < 250)
    {
        ...
        null(cohendo, ARRLLP_dim);
        null(VFendo, ARRLLP_dim);

        copy(VF_final, VF_final_old, ARRLL_dim);
                            ...
                            for (portfoliochoice_index = 0; portfoliochoice_index < size_portfoliochoice + 1; portfoliochoice_index++)
                            {
                                ...
                                for (risk_indexnext = 0; risk_indexnext < size_risk; risk_indexnext++)
                                {
                                    for (laborincome_indexnext = 0; laborincome_indexnext < size_laborincome; laborincome_indexnext++)
                                    {
                                        ...
                                        tempnext += risk_labor_trans[state_current][state_next] * VF[index_ARRLLP(asset_index, risk_indexnext, risk_index, laborincome_indexnext, laborincome_index, portfoliochoice_index)];
                                        dtempnext += risk_labor_trans[state_current][state_next] * dVF[index_ARRLLP(asset_index, risk_indexnext, risk_index, laborincome_indexnext, laborincome_index, portfoliochoice_index)];
                                    }
                                }
                        ...
                        for (laborincome_pre_index = 0; laborincome_pre_index < size_laborincome; laborincome_pre_index++)
                        {
                            ...
                            for (asset_index = 0; asset_index < size_asset; asset_index++)
                            {
                                ...
                                if (cohexo < cohendo[index_ARRLLP(0, risk_index, risk_pre_index, laborincome_index, laborincome_pre_index, portfoliochoice_index)])
                                {
                                    ...
                                    // here is the place: utility of wealth play a role

                                    VF[index_ARRLLP(asset_index, risk_index, risk_pre_index, laborincome_index, laborincome_pre_index, portfoliochoice_index)] += Uw(K[0]);
                                }
                                ...
                                if (cohexo >= cohendo[index_ARRLLP(0, risk_index, risk_pre_index, laborincome_index, laborincome_pre_index, portfoliochoice_index)])
                                {
                                    ...
                                    VF[index_ARRLLP(asset_index, risk_index, risk_pre_index, laborincome_index, laborincome_pre_index, portfoliochoice_index)] += Uw(K[asset_index]);
                                }
                            }
                        }
                    }
...
void SIMULATION(double *save, double *dist, double *capitalout, double K[size_asset])
{
    ...
    distold = (double *)calloc((ARRLL_dim), sizeof(double));
...
int main()
{
    ...
    double capital1, capital0, PIB, critprice, taxL, welfare, rrate, temp, wagerate; // for equilibrium
    ...
    dVF = (double *)calloc((ARRLLP_dim), sizeof(double));  // value function derivative
    save = (double *)calloc((ARRLLP_dim), sizeof(double)); // value function

    double *VF_final, *dVF_final, *save_final;
    double *distin_final, *distout_final, capitalout; // for simulation

    VF_final = (double *)calloc((ARRLL_dim), sizeof(double));  // value function
    dVF_final = (double *)calloc((ARRLL_dim), sizeof(double)); // value function derivative

    save_final = (double *)calloc((ARRLL_dim), sizeof(double));
    Portfolio = (double *)calloc((ARRLL_dim), sizeof(double));
    // cons = (double *)calloc((ARRLLP_dim), sizeof(double));
    distin_final = (double *)calloc((ARRLL_dim), sizeof(double));
    distout_final = (double *)calloc((ARRLL_dim), sizeof(double));

    null(VF, ARRLLP_dim);
    null(dVF, ARRLLP_dim);

    null(VF_final, ARRLL_dim);
    null(dVF_final, ARRLL_dim);
    null(save_final, ARRLL_dim);
    null(distin_final, ARRLL_dim);
    null(distout_final, ARRLL_dim);
            ...
            for (risk_index = 0; risk_index < size_risk; risk_index++)
            {
                for (risk_pre_index = 0; risk_pre_index < size_risk; risk_pre_index++)
                {
                    for (laborincome_index = 0; laborincome_index < size_laborincome; laborincome_index++)
                    {
                        ...
                        {
                            state_current = risk_index * size_risk + laborincome_index;
                            temp = wagerate * laborincome_states[state_current] + (1 + r_f + pi + risk_states[state_current]) * K[asset_index];
                            VF[index_ARRLLP(asset_index, risk_index, risk_pre_index, laborincome_index, laborincome_pre_index, portfoliochoice_index)] = U(temp) + Uw(K[asset_index]); // REQUIERE TO BE INCREASING IN K (the case here)
                        }
            ...
            for (risk_pre_index = 0; risk_pre_index < size_risk; risk_pre_index++)
            {
                for (laborincome_index = 0; laborincome_index < size_laborincome; laborincome_index++)
                {
                    for (laborincome_pre_index = 0; laborincome_pre_index < size_laborincome; laborincome_pre_index++)
                    {
                        state_current = risk_index * size_risk + laborincome_index;
                        temp = wagerate * laborincome_states[state_current] + (1 + r_f + pi + risk_states[state_current]) * K[asset_index];
                        VF_final[index_ARRLL(asset_index, risk_index, risk_pre_index, laborincome_index, laborincome_pre_index)] = U(temp) + Uw(K[asset_index]); // REQUIERE TO BE INCREASING IN K (the case here)
                    }
                }
            }
        }
