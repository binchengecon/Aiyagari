...
{
    double *distold, critdist, distverif, weight, omegatemp, currentcoh, temp1, temp2, temp3, temp4;
                        ...
                        for (risk_indexnext = 0; risk_indexnext < size_risk; risk_indexnext++)
                        {
                            for (laborincome_indexnext = 0; laborincome_indexnext < size_laborincome; laborincome_indexnext++)
                            {
                                for (z = 0; z < length_z; z++)
                                {
                                    ...
                                    if (weight > 1)
                                    {
                                        weight = 1.0 + (weight - 1.0) / 500;
                                        // weight = 1.0;

                                    } // this is a small correction for potential approx in the extrapolation at the top.
                                    if (isave < 0 | weight < 0.0)
                                    {
...
#pragma omp parallel for reduction(+:distverif) reduction(max:critdist) private(asset_index, risk_index, laborincome_index)
    ...
    //     for (laborincome_index = 0; laborincome_index < size_laborincome; laborincome_index++)
    //     {
    //         error += dist[index(0, risk_index, laborincome_index)] * A[asset_index];
    //     }
    // }
    // if (error == 1)
    // {
