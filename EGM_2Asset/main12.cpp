...
const int size_asset = 1000; // number of grid points
const int size_risk = 7;     // number of productivity classes
const int size_laborincome = 7;
const int size_portfoliochoice = 1000;
                ...
                for (riskpre_index = 0; riskpre_index < size_risk; riskpre_index++)
                {
                    ...
                    for (laborincome_index = 0; laborincome_index < size_laborincome; laborincome_index++)
                    {
                        for (portfoliochoice_index = 0; portfoliochoice_index < size_portfoliochoice + 1; portfoliochoice_index++)
                        {
                            ...
                            VFendo[index_ARRLP(asset_index, risk_index, riskpre_index, laborincome_index, portfoliochoice_index)] = U(cohendo[index_ARRLP(asset_index, risk_index, riskpre_index, laborincome_index, portfoliochoice_index)] - K[asset_index]) + betapar * tempnext;
                        }
                    }
                }
            }
...
int main()
{
        ...
        for (asset_index = 0; asset_index < size_asset; asset_index++)
        {
            for (risk_index = 0; risk_index < size_risk; risk_index++)
            {
                for (riskpre_index = 0; riskpre_index < size_risk; riskpre_index++)
                {
                    for (laborincome_index = 0; laborincome_index < size_laborincome; laborincome_index++)
                    {
                        VF[index_ARRLP(asset_index, risk_index, riskpre_index, laborincome_index, portfoliochoice_index)] = U(wagerate * laborincome_states[laborincome_index] + (1 + r_f + pi + risk_states[risk_index]) * K[asset_index]); // REQUIERE TO BE INCREASING IN K (the case here)
    ...
    for (asset_index = 0; asset_index < size_asset; asset_index++)
    {
        for (risk_index = 0; risk_index < size_risk; risk_index++)
        {
            for (riskpre_index = 0; riskpre_index < size_risk; riskpre_index++)
            {
                for (laborincome_index = 0; laborincome_index < size_laborincome; laborincome_index++)
                {
                    VF_final[index_ARRL(asset_index, risk_index, riskpre_index, laborincome_index)] = U(wagerate * laborincome_states[laborincome_index] + (1 + r_f + pi + risk_states[risk_index]) * K[asset_index]); // REQUIERE TO BE INCREASING IN K (the case here)
                }
            }
        }
    }
    ...
    std::string filename_dist = "csv\\dist12ad,pe=e-9,std=0.2,premium=" + std::to_string(pi) + ",wage=" + std::to_string(wagerate) + ",rf=" + std::to_string(r_f) + ",Psize=" + std::to_string(size_portfoliochoice) + ",rho_c=" + std::to_string(rhopar) + ",Ksize=" + std::to_string(size_asset) + ",relaxVF=" + std::to_string(relaxVF) + ",beta=" + std::to_string(betapar) + ".csv ";
    std::string filename_policy = "csv\\policy12ad,pe=e-9,std=0.2,premium=" + std::to_string(pi) + ",wage=" + std::to_string(wagerate) + ",rf=" + std::to_string(r_f) + ",Psize=" + std::to_string(size_portfoliochoice) + ",rho_c=" + std::to_string(rhopar) + ",Ksize=" + std::to_string(size_asset) + ",relaxVF=" + std::to_string(relaxVF) + ",beta=" + std::to_string(betapar) + ".csv ";
    std::string filename_VF = "csv\\VF12ad,pe=e-9,std=0.2,premium=" + std::to_string(pi) + ",wage=" + std::to_string(wagerate) + ",rf=" + std::to_string(r_f) + ",Psize=" + std::to_string(size_portfoliochoice) + ",rho_c=" + std::to_string(rhopar) + ",Ksize=" + std::to_string(size_asset) + ",relaxVF=" + std::to_string(relaxVF) + ",beta=" + std::to_string(betapar) + ".csv ";
    std::string filename_Port = "csv\\Portfolio12ad,pe=e-9,std=0.2,premium=" + std::to_string(pi) + ",wage=" + std::to_string(wagerate) + ",rf=" + std::to_string(r_f) + ",Psize=" + std::to_string(size_portfoliochoice) + ",rho_c=" + std::to_string(rhopar) + ",Ksize=" + std::to_string(size_asset) + ",relaxVF=" + std::to_string(relaxVF) + ",beta=" + std::to_string(betapar) + ".csv ";
