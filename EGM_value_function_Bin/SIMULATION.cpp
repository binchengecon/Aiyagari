...
void SIMULATION(double *save, double *startdist, double *enddist)
{
    ...
    for (y = 0; y < maxygrid; y++)
    {
        for (i = 0; i < maxigrid; i++)
        {
            ...
            if (save[inx(i, y)] > Gridmax && save[inx(i, y)] < Gridmax + 0.00000001)
            {
                save[inx(i, y)] = Gridmax - 0.00000001;
            }
            if (save[inx(i, y)] < Gridmin && save[inx(i, y)] > Gridmin - 0.00000001)
            {
                save[inx(i, y)] = Gridmin;
            }
