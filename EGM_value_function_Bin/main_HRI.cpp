...
const double nsstates[7] = {exp(-0.030619), exp(-0.020412), exp(-0.010206), exp(0.000000), exp(0.010206), exp(0.020412), exp(0.030619)};
const double nstrans[7][7] = {
...
void SIMULATION_WF(double *save, double *dist, double *capitalout, double K[size_k])
{
    ...
    while (critdist > epsdist)
    {
        ...
        //             // std::cout<< dist[inx(i, y, n)];
        //         }
        //     }
        // }

        // std::cout << "distribution total 1 = " << distverif << "\n";
                ...
                for (n = 0; n < size_n; n++)
                {
                    if (distold[inx(i, y, n)] > 0)
                    {
                        ...
                        for (ynext = 0; ynext < size_m; ynext++)
                        {
                            for (nnext = 0; nnext < size_n; nnext++)
                            {
                                dist[inx(isave, ynext, nnext)] += (1.0 - weight) * mstrans[y][ynext] * nstrans[n][nnext] * distold[inx(i, y, n)];
                                dist[inx(min(isave + 1, size_k - 1), ynext, nnext)] += (weight)*mstrans[y][ynext] * nstrans[n][nnext] * (distold[inx(i, y, n)]);
                            }
                        }
                    }
                }
        ...
        // std::cout << "distribution crit 2 = " << critdist << "\n";

        // std::cout << "distribution total 2 = " << distverif << "\n";
    }
    ...
    *capitalout = 0.0;
    ...
    // std::cout << "Capitalout 2 = " <<*capitalout;
}
