...
double FOC_nu(const double nu, void *params){
    ...
    for(z=0;z<length_z;z++){                // investment shock on private equity
        ...
        psinext = nu*(A[i])*(1.10*Z[z]) + (1.0-nu)*(A[i])*(1.04);
...
//        }
        ...
        if(weight > 1){weight = 1.0 + (weight-1.0)/1;}
...
//                        if(params_NU.NU_stop == 1){
//                            val_try = (FOC_nu(NU[x((i-1),y,t)],&params_NU));
//                            max_GVF = val_try;
//                            NUmax   = NU[x((i-1),y,t)];
//                            break;
//                        }
